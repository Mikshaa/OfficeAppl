...
checkForFormula = ['A', 'I', 'i', 'U', 'E', 'X', 'K', 'L', 'O', 'S', 'G', 'У', '.', 'W', 'З', 'В', 'М', 'О', 'Е', 'Р',
                   'С', 'А', 'Л', 'К', 'Т', 'Ь', 'И']
...
def generateDeviceOutuput(mode='device', contractAmount=None):
    ...
    if curDeviceCode == '':
        return
    peData = pe.DataFrame(pe.read_excel(f'{inputFilesPath}/{curFile}'),
                          columns=['Наименование ВП', 'Количество', 'Примечание'])
...
def genarateContractOutput():
    ...
    if curSecondMode == 'Separate':
        ...
        for i in wbDataFrames:
            for j in range(len(i)):
                if i[0][j] in wbDataFinalList:
                    wbValues[wbDataFinalList.index(i[0][j])] = float(i[1][j]) + float(
                        wbValues[wbDataFinalList.index(i[0][j])])
                else:
        ...
        wb_final.save(
            f'{outputFilesPath}/{contractFilePath[contractFilePath.rfind("/") + 1:contractFilePath.rfind(".")]}.xlsx')
